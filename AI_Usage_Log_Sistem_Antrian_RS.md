...
### Interaksi 2 (Minggu 9)
- **Prompt:** "Buatkan problem statement dari Sistem Antrian Rumah Sakit/Puskesmas sesuai template [template problem statement dilampirkan]."
- **Respons AI:** Menyusun draf problem statement (masalah, siapa terpengaruh, solusi, fitur utama) berdasarkan 9 fitur yang sudah disepakati sebelumnya.
- **Yang digunakan:** Seluruh struktur problem statement dipakai sebagai draf awal proposal.
- **Modifikasi:** Tidak ada modifikasi signifikan — kelompok memverifikasi kesesuaian dengan konteks nyata sebelum finalisasi.
- **Pelajaran:** Problem statement yang baik harus menyebut stakeholder spesifik (bukan hanya "pengguna"), supaya solusi dan fitur yang diajukan punya alasan yang jelas.

### Interaksi 3 (Minggu 10)
...
### Interaksi 7 (Minggu 11)
- **Prompt:** "Buat Desain User Interface (Console-Based)."
- **Respons AI:** Mockup tampilan 10 layar menu (menu utama, pendaftaran, panggil pasien, dst.) dengan format kotak ASCII.
- **Yang digunakan:** Sebagian besar isi teks dan alur tiap layar.
- **Modifikasi:** Bentuk kotak menu utama diminta disesuaikan ulang persis dengan rancangan kelompok sendiri (border ganda `╔═╗`/`╠═╣`).
- **Pelajaran:** Karakter box-drawing (`┌─┐`) tidak selalu tampil rapi di semua terminal — perlu diuji langsung, bukan diasumsikan.

### Interaksi 8 (Minggu 12)
...
### Interaksi 9 (Minggu 12)
- **Prompt:** "Untuk estimasi waktu tunggu, itu berarti menampilkan pasien yang menunggu? Bukan waktu tunggu dalam bentuk menit?"
- **Respons AI:** Menjelaskan bahwa fitur 8 memang menghasilkan estimasi dalam menit, dengan posisi antrian sebagai konteks pendukung, bukan tujuan utama fitur.
- **Yang digunakan:** Penjelasan dipakai untuk memahami logika kode sebelum melanjutkan ke fitur berikutnya.
- **Modifikasi:** Tidak ada perubahan kode dari interaksi ini — murni klarifikasi pemahaman.
- **Pelajaran:** Penting memverifikasi pemahaman terhadap output kode AI sebelum melanjutkan, supaya tidak salah asumsi saat presentasi.

### Interaksi 10 (Minggu 12)
...
- **Modifikasi:** Fitur JSON persistence **dihapus kembali** pada interaksi berikutnya (lihat Interaksi 11) atas keputusan kelompok sendiri.
